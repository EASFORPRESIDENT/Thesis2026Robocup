...
def main():
    ...
    HIDDEN_DIM = 64
    WEIGHTS_PATH = PROJECT_ROOT / f"2v1_noicy_T2/agent_weights.pth"
    ...
    MAX_GRAD = 5
    training_step = 0
    Eval_time_steps = 500 #Number of time steps between evaluations during training
    Eval_interval = 100 #Number of episodes to average over during evaluation
    ...
    if training:
        ...
        agent_net = RecurrentAgentNetwork(OBS_DIM, HIDDEN_DIM, N_ACTIONS) 
        state_dict = torch.load(WEIGHTS_PATH, map_location="cpu") #Load network from file
    ...
    for i in range(N_AGENTS):
        p = mp.Process(target=run_agent, args=(i,agent_net,queue,barrier,training,N_ACTIONS,Epsilon,Eval_flag,Eval_interval)) 
    ...
    while training:
        ...
        if Eval_flag.value == False:
            ...
            if training_step % 6000 == 0 and training_step > 0 and LEARNING_RATE > 5e-6:
            ...
            dup_cap = 20
            replay_buffer.end_episode(min(dup_cap, round(1/average_goal_rate) if average_goal_rate > 0 else dup_cap)) # Prioritize episodes with positive reward in replay buffer by duplicating them.
            ...
            training_step += 1
            if training_step == 100000: #Stop training after 30000 training steps
